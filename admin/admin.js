...
(() => {
  ...
  async function api(method, url, body) {
    ...
    // Fallback: Handle Supabase direct database calls
    if (url === '/api/projects' && method === 'GET') {
      const res = await sb.from('projects').select('*').order('sort_order', { ascending: true });
    ...
    if (url.startsWith('/api/admin/projects/') && method === 'PATCH') {
      const id = url.split('/').pop();
      const updates = { ...body };
      if ('desc' in updates) { updates.description = updates.desc; delete updates.desc; }
      if ('heroSlot' in updates) { updates.hero_slot = updates.heroSlot; delete updates.heroSlot; }
      if ('heroTitle' in updates) { updates.hero_title = updates.heroTitle; delete updates.heroTitle; }
      if ('heroTag' in updates) { updates.hero_tag = updates.heroTag; delete updates.heroTag; }
      if ('showPhoto' in updates) { updates.show_photo = updates.showPhoto; delete updates.showPhoto; }
      if ('showVideo' in updates) { updates.show_video = updates.showVideo; delete updates.showVideo; }
      if ('showGraphic' in updates) { updates.show_graphic = updates.showGraphic; delete updates.showGraphic; }
      const res = await sb.from('projects').update(updates).eq('id', id);
      if (res.error) throw res.error;
      return { ok: true };
    }
    ...
    if (url === '/api/admin/projects' && method === 'POST') {
      ...
      const newProj = {
        ...
        thumb: body.thumb || body.image || null,
        featured: Boolean(body.featured),
        hero_slot: body.heroSlot !== undefined ? body.heroSlot : null,
        hero_title: body.heroTitle || null,
        hero_tag: body.heroTag || null,
        badges: body.badges || [],
        alt: body.alt || null,
        sort_order: state.projects.length,
        show_photo: body.showPhoto !== undefined ? body.showPhoto : true,
        show_video: body.showVideo !== undefined ? body.showVideo : true,
        show_graphic: body.showGraphic !== undefined ? body.showGraphic : true
      };
      const res = await sb.from('projects').insert(newProj);
      if (res.error) throw res.error;
      return { id };
    }
    ...
    if (url === '/api/admin/projects/order' && method === 'PUT') {
      const { ids } = body;
      for (let i = 0; i < ids.length; i++) {
        await sb.from('projects').update({ sort_order: i }).eq('id', ids[i]);
      }
      return { ok: true };
    }

    if (url.startsWith('/api/admin/categories/') && method === 'PATCH') {
      const slug = url.split('/').pop();
      const res = await sb.from('categories').update({ label: body.label }).eq('slug', slug);
      if (res.error) throw res.error;
      return { ok: true };
    }
    ...
    if (url.startsWith('/api/admin/categories/') && method === 'DELETE') {
      const slug = url.split('/').pop();
      const res = await sb.from('categories').delete().eq('slug', slug);
      if (res.error) throw res.error;
      return { ok: true };
    }

    if (url.includes('/image') && (method === 'POST' || method === 'PUT')) {
      const match = url.match(/\/api\/projects\/([^/]+)\/image/);
      if (match && body instanceof FormData) {
        const id = match[1];
        const file = body.get('image');
        if (file) {
          const ext = file.name.split('.').pop() || 'jpg';
          const filePath = `uploads/${id}-${Date.now()}.${ext}`;
          const uploadRes = await sb.storage.from(window.SUPABASE_CONFIG.storageBucket).upload(filePath, file, { upsert: true });
          if (uploadRes.error) throw uploadRes.error;
          const { data: pubData } = sb.storage.from(window.SUPABASE_CONFIG.storageBucket).getPublicUrl(filePath);
          await sb.from('projects').update({ image: pubData.publicUrl, thumb: pubData.publicUrl }).eq('id', id);
          return { ok: true, url: pubData.publicUrl };
        }
      }
    }
    ...
    if (url.includes('/image') && method === 'DELETE') {
      ...
      if (match) {
        const id = match[1];
        await sb.from('projects').update({ image: null, thumb: null }).eq('id', id);
        return { ok: true };
      }
    }
  ...
  $('#login-form').addEventListener('submit', async (ev) => {
    ...
    if (!user) {
      err.textContent = 'Invalid credentials or login failed';
      err.hidden = false;
      return;
    }
      ...
      if (sb) {
        try {
          ...
          if (data && data.session && data.session.user) {
            user = { username: data.session.user.email, id: data.session.user.id };
          }
        } catch (_) {}
      }
    }
